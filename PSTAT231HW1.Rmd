...
# https://stackoverflow.com/questions/57107461/boxplot-by-group-and-then-column-in-r
mpg_dt <- setDT(copy(mpg))
mpg_plot <- melt(mpg_dt,measure.vars = c("cty","hwy","displ"),value.name = "val",variable.name = "var")
...
mpg_numeric <- mpg %>%
  ...
  mutate_if(sapply(mpg, is.factor), as.numeric)
M <- cor(mpg_numeric)
